'''
    * 크롤링
        1) urllib 라이브러리 이용한 웹 페이지 크롤링
            - urllib.request 모듈
                - url을 열어서 내용을 읽어 들이는 모듈이다.
                - 웹을 통하여 데이터를 요청하는 기능이다.
                - http 클라이언트의 인터페이스를 위한 함수 및 클래스를 제공한다.
                    - urlretrieve(url, savename) : url을 savename이라는 이름의 파일로 다운로드한다.
                    - urlopen() : 네트워크를 통해 원격 객체를 읽고 메모리에 올리는 역할을 수행한다.
